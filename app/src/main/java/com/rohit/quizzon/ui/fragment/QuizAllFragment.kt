package com.rohit.quizzon.ui.fragment

import android.os.Bundle
import android.view.LayoutInflater
import android.view.View
import android.view.ViewGroup
import androidx.fragment.app.Fragment
import androidx.fragment.app.viewModels
import androidx.lifecycle.lifecycleScope
import androidx.navigation.fragment.findNavController
import androidx.navigation.fragment.navArgs
import androidx.recyclerview.widget.LinearLayoutManager
import com.rohit.quizzon.data.model.response.QuizResponse
import com.rohit.quizzon.databinding.FragmentQuizAllBinding
import com.rohit.quizzon.ui.adapter.QuizAdapter
import com.rohit.quizzon.ui.viewmodels.QuizListViewModel
import com.rohit.quizzon.utils.NetworkResponse
import com.rohit.quizzon.utils.autoCleaned
import com.rohit.quizzon.utils.listener.QuizClickListener
import com.rohit.quizzon.utils.shortToast
import dagger.hilt.android.AndroidEntryPoint
import kotlinx.coroutines.flow.collectLatest

@AndroidEntryPoint
class QuizAllFragment : Fragment(), QuizClickListener {

    private var binding: FragmentQuizAllBinding by autoCleaned()
    private val quizAdapter: QuizAdapter by autoCleaned { QuizAdapter(this) }
    private val args: QuizAllFragmentArgs by navArgs()
    private val quizListViewModel: QuizListViewModel by viewModels()

    override fun onCreateView(
        inflater: LayoutInflater,
        container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View {

        binding = FragmentQuizAllBinding.inflate(inflater, container, false)
        startShimmer()
        binding.rvRecyclerView.apply {
            layoutManager = LinearLayoutManager(requireContext())
            adapter = quizAdapter
        }
        quizListViewModel.getQuizByCategory(args.categoryId)

        loadData()
        return binding.root
    }

    private fun loadData() {
        viewLifecycleOwner.lifecycleScope.launchWhenStarted {
            quizListViewModel.quizList.collectLatest {
                when (it) {
                    is NetworkResponse.Success -> {
                        quizAdapter.submitList(it.data)
                        stopShimmer()
                    }
                    is NetworkResponse.Failure -> {
                        it.message?.let { it1 -> shortToast(it1) }
                        stopShimmer()
                    }
                    is NetworkResponse.Loading -> {
                        startShimmer()
                    }
                }
            }
        }
    }

    override fun quizClickListener(quizListBody: QuizResponse) {
        findNavController().navigate(
            QuizAllFragmentDirections.actionQuizListFragmentToQuizFragment(
                quizListBody
            )
        )
    }

    private fun startShimmer() {
        binding.apply {
            homeShimmerEffect.visibility = View.VISIBLE
            homeShimmerEffect.startShimmer()
            rvRecyclerView.visibility = View.GONE
        }
    }

    private fun stopShimmer() {
        binding.apply {
            homeShimmerEffect.visibility = View.GONE
            homeShimmerEffect.stopShimmer()
            rvRecyclerView.visibility = View.VISIBLE
        }
    }
}
